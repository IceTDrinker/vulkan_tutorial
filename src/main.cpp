...
private:
    static void framebufferResizeCallback(GLFWwindow* t_window, int /*t_width*/, int /*t_height*/) noexcept
    {
        auto app = reinterpret_cast<HelloTriangleApplication*>(glfwGetWindowUserPointer(t_window));
...
#pragma warning(pop)
    ...
    bool isDeviceSuitable(VkPhysicalDevice t_device)
    {
        ...
        vkGetPhysicalDeviceFeatures(t_device, &supportedFeatures);
    ...
    void createBuffer(VkDeviceSize t_size, VkBufferUsageFlags t_usage, VkMemoryPropertyFlags t_properties, VkBuffer& t_buffer, VkDeviceMemory& t_bufferMemory)
    {
    ...
    void copyBufferToImage(VkBuffer t_buffer, VkImage t_image, uint32_t t_width, uint32_t t_height)
    {
        VkCommandBuffer commandBuffer = beginSingleTimeCommands();
        {
        ...
        VkDeviceMemory& t_imageMemory)
    {
        ...
        if (vkCreateImage(m_device, &imageInfo, nullptr, &t_image) != VK_SUCCESS)
        {
        ...
        vkBindImageMemory(m_device, t_image, t_imageMemory, 0);
    }
    ...
    void createTextureImage()
    {
        ...
        VkDeviceSize imageSize = static_cast<uint64_t>(texWidth) * static_cast<uint64_t>(texHeight) * 4ull;
    ...
    VkImageView createImageView(VkImage t_image, VkFormat t_format)
    {
        VkImageViewCreateInfo viewInfo{};
        viewInfo.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
        viewInfo.image = t_image;
        viewInfo.viewType = VK_IMAGE_VIEW_TYPE_2D;
        viewInfo.format = t_format;
    ...
    void updateUniformBuffer(uint32_t t_currentImage)
    {
